...
func (m *MediaEngine) updateCodecParameters(remoteCodec RTPCodecParameters, typ RTPCodecType) error {
	...
	for _, localCodec := range codecs {
		if strings.EqualFold(localCodec.RTPCodecCapability.MimeType, remoteCodec.RTPCodecCapability.MimeType) &&
			localCodec.RTPCodecCapability.SDPFmtpLine == remoteCodec.RTPCodecCapability.SDPFmtpLine {
